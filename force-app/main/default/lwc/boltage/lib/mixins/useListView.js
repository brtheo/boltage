...
export const useListView =
  ...
  (clazz) =>
    class extends useSObject({
      objectApiName,
      fields: (self) =>
        self.ListViewFields !== undefined
      ...
      get SortedBy() {
        ...
        return sort?.by !== undefined ? [`${sign}${sort?.by}`] : undefined;
      }

      get __LIST_MXN_DONE__() {
        return (
          this.ListViewFields !== undefined &&
      ...
      get __LIST_MXN_RECORDS_PARAM__() {
        return this.__LIST_MXN_RECORD_IDS__ !== undefined &&
          this.ListViewFields !== undefined
          ? [
              {
                recordIds: this.__LIST_MXN_RECORD_IDS__,
                fields: this.ListViewFields?.concat(
                  [{ objectApiName, fieldApiName: "Id" }],
                  optionalFields ?? [],
                ),
              },
            ]
      ...
      get __LIST_MXN_RECORD_IDS__() {
        return this.__LIST_MXN_RECORDS_WIRED_RESULTS__?.data?.records instanceof
          Array &&
          this.__LIST_MXN_RECORDS_WIRED_RESULTS__?.data?.records.length >= 1
          ? this.__LIST_MXN_RECORDS_WIRED_RESULTS__?.data?.records.map(
              ({ id }) => id,
            )
          : [];
      }
      get NextPageToken() {
        return (
          this.__LIST_MXN_RECORDS_WIRED_RESULTS__?.data?.nextPageToken ??
          undefined
        );
      }
      get PreviousPageToken() {
        return (
          this.__LIST_MXN_RECORDS_WIRED_RESULTS__?.data?.previousPageToken ??
          undefined
        );
      }
      get PageToken() {
      ...
      get PreviousFirstRowId() {
        return this.previousFirstRowId;
      }
      set PreviousFirstRowId(v) {
        this.previousFirstRowId = this.currentFirstRowId;
        if (v === this.actualFirstRowId) this.currentFirstRowId = v;
        if (
          this.previousFirstRowId &&
          this.actualFirstRowId !== this.previousFirstRowId
        )
          this.__LOADING_NEW_PAGE__ = false;
      }
      get ListViewLabel() {
        return this.__LIST_MXN_INFO_WIRED_RESULTS__?.data?.label;
      }

      get ListViewFields() {
        return this.__LIST_MXN_INFO_WIRED_RESULTS__?.data?.displayColumns.map(
          ({ fieldApiName }) => ({
            fieldApiName,
            objectApiName,
          }),
      ...
      get ListViewData() {
        ...
        const ready =
          this.__LIST_MXN_RECORDS_WIRED_RESULTS__?.data?.records instanceof
            Array &&
          this.__LIST_MXN_RECORDS_WIRED_RESULTS__?.data?.records.length >= 1;
        const data = this.__LIST_MXN_RECORDS__?.data?.results?.map(
          ({ result: { fields } }) =>
            Object.fromEntries(
              Object.entries(fields).map(
                ([fieldApiName, { displayValue, value }]) => [
                  fieldApiName,
      ...
      get ListViewColumns() {
        ...
        const DATE_FIELD = (field) =>
          ["Date", "DateTime"].includes(get("dataType", field));
        ...
        return SOBJECT_INFO !== undefined
          ? this.ListViewFields?.map(({ fieldApiName }) => {
              ...
              return (
                this?.customColumns?.[fieldApiName] ?? {
                  type:
                    dataTypeToColumnType[
                      get("dataType", fieldApiName, objectApiName)
                    ] ?? "text",
                  fieldName: fieldApiName,
                  label: get("label", fieldApiName, objectApiName),
                  sortable: true,
